...
# Temporary release safety gate: R8 breaks Expo Record option conversion in
# the published 0.20.1 AAB. Do not silently re-enable shrinking before a
# Play-installed release verifies SecureStore and DocumentPicker end-to-end.
grep -Fq 'android.enableMinifyInReleaseBuilds=false' "$GRADLE_PROPERTIES" ||
  fail 'release minification must remain disabled pending Expo Record verification'
grep -Fq 'android.enableShrinkResourcesInReleaseBuilds=false' "$GRADLE_PROPERTIES" ||
  fail 'release resource shrinking must remain disabled pending Expo Record verification'
...
printf 'Building unminified release bundle with JDK 25 native access enabled...\n'
(
