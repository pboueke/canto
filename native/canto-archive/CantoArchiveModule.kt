...
class CantoArchiveModule(
                ...
                ZipFile(archive).use { zip ->
                    ...
                    zip.getInputStream(entry).use { input ->
                        FileOutputStream(outputFile).use { output ->
                            written =
                                copyEntryAndVerifyCrc(input, output, entry) {
    ...
    private fun openSource(value: String): InputStream {
        ...
        return when (uri.scheme) {
            ...
            "file" -> {
                FileInputStream(File(uri.path ?: throw IllegalArgumentException("Invalid file URI")))
            }
    ...
    private fun readExactlyBounded(
        input: InputStream,
        entry: ZipEntry,
    ): ByteArray {
